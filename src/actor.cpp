...
ShadersManager *Actor::shadersManager = nullptr;
PhysicsManager *Actor::physicsManager = nullptr;

Actor::Actor()
{
    shadersManager->compileSysShaders();
    registerName("Actor");
}
        ...
        if (collisionsAmount > 0)
        {
            ...
            Body *body = bodyInterface.CreateBody(
                BodyCreationSettings(compoundRoot,
                                     ...
                                     (EMotionType)mType,
                                     physicsManager->getMovingLayerId()));
...
void Actor::onDestroy()
{
    ...
    if (physicsRoot && system)
    {
        Body *rootEntity = (Body *)physicsRoot;
        PhysicsSystem *physicsSystem = (PhysicsSystem *)system->system;
        BodyInterface &bodyInterface = physicsSystem->GetBodyInterface();
        bodyInterface.RemoveBody(rootEntity->GetID());
        delete rootEntity;
    }
}
...
void Actor::setShadersManager(ShadersManager *shadersManager)
{
    Actor::shadersManager = shadersManager;
}

void Actor::setPhysicsManager(PhysicsManager *physicsManager)
{
    Actor::physicsManager = physicsManager;
}
